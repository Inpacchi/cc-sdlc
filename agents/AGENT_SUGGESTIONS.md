...
**Creating agents:** Always invoke `/sdlc-create-agent` to create agents. Do NOT write agent files directly — the skill validates frontmatter, formats description `<example>` blocks, and scaffolds the system prompt. See `[sdlc-root]/templates/agent-template.md` for the structural pattern the skill produces.
...
All agents follow the structure in `[sdlc-root]/templates/agent-template.md`. To create a new agent, use `/sdlc-create-agent` — it handles frontmatter validation, description formatting with `<example>` blocks, and system prompt scaffolding. Then add your project-specific context (file paths, tech stack, conventions).
